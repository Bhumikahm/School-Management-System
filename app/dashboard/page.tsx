...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState<any>({ totalUsers: 0, totalSchools: 0, activePercentage: 0 });
  ...
  useEffect(() => {
    ...
    fetch('/api/auth/verify', {
      ...
      .then(data => {
        if (data.user) {
          setUser(data.user);
          // Fetch stats after user is verified
          fetchStats(token);
        } else {
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
            ...
            <Card className="border border-white/10 bg-white/5 backdrop-blur-xl hover:bg-white/10 transition-all duration-300">
              <CardContent className="p-6 text-center">
                ...
                <p className="text-2xl font-bold text-purple-400">{stats.totalSchools}</p>
              </CardContent>
            </Card>
            ...
            <Card className="border border-white/10 bg-white/5 backdrop-blur-xl hover:bg-white/10 transition-all duration-300">
              <CardContent className="p-12 text-center">
                ...
                <p className="text-2xl font-bold text-indigo-400">{stats.activePercentage}%</p>
                <p className="text-sm text-gray-400 mt-1">{stats.verifiedUsers} verified</p>
              </CardContent>
            </Card>
          </div>
        </div>
